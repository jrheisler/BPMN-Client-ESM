...
test('app.css uses CSS variables for overlays', async () => {
  ...
  assert.ok(/\.djs-element:not\(\.djs-label\)\.bpmn-addOn-highlight .djs-visual > :nth-child\(1\)\s*\{[^}]*stroke:\s*var\(--accent\);[^}]*fill:\s*var\(--accent\);[^}]*filter:\s*drop-shadow\(0 0 6px var\(--accent\)\)/s.test(css));
  assert.ok(/\.djs-element:not\(\.djs-label\)\.drop-ok .djs-visual > :nth-child\(1\)\s*\{[^}]*stroke:\s*var\(--ok\);[^}]*fill:\s*var\(--ok\);[^}]*filter:\s*drop-shadow\(0 0 6px var\(--ok\)\)/s.test(css));
